...
#
# ERP's CI runners are all GitHub-hosted `ubuntu-latest` (linux x86_64) on
# Python 3.12 (see erp-lock.yml's resolve job and ci.yml) — no self-hosted
# runner is in scope here, unlike the sibling repository this script was
# ported from — so this pins for that one target.
#
