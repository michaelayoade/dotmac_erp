...
WORKTREE_DIR=/home/dotmac/projects/dotmac_erp/.worktrees/fix-deps-008
...
LOG_FILE=/home/dotmac/projects/dotmac_erp/.seabone/logs/fix-deps-008.log
TASK_ID=fix-deps-008
DESCRIPTION=Replace\ passlib\ with\ direct\ bcrypt:\ passlib\ 1.7.4\ is\ abandoned\ \(last\ release\ 2020\)\ and\ has\ compatibility\ breaks\ with\ bcrypt\>=4.0.0\ where\ it\ misreads\ the\ version\ string\ and\ may\ silently\ fall\ back\ to\ insecure\ hashing.\ Migrate\ to\ direct\ bcrypt\ usage.\ Steps:\ 1\)\ Find\ all\ passlib\ usage\ in\ app/\ \(search\ for\ \'passlib\'\,\ \'CryptContext\'\,\ \'pwd_context\'\).\ 2\)\ Replace\ with:\ import\ bcrypt\;\ hash\ =\ bcrypt.hashpw\(password.encode\(\)\,\ bcrypt.gensalt\(\)\).decode\(\)\;\ verify\ =\ bcrypt.checkpw\(password.encode\(\)\,\ stored_hash.encode\(\)\).\ 3\)\ Remove\ passlib\ from\ pyproject.toml\,\ add\ bcrypt\>=4.0.0.\ 4\)\ Run\ \`poetry\ update\`\,\ \`make\ lint\`\,\ \`pytest\ -x\ --tb=short\`.\ The\ password\ hashing\ is\ used\ in\ app/services/auth/\ for\ user\ account\ management.
