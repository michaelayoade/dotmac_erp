...
WORKTREE_DIR=/home/dotmac/projects/dotmac_erp/.worktrees/fix-security-c1-3
...
LOG_FILE=/home/dotmac/projects/dotmac_erp/.seabone/logs/fix-security-c1-3.log
TASK_ID=fix-security-c1-3
DESCRIPTION=Security\ fix:\ DNS\ rebinding\ vulnerability\ in\ webhook\ validation.\ File:\ app/services/finance/automation/workflow.py\,\ around\ line\ 214.\ The\ _validate_webhook_target\(\)\ function\ resolves\ the\ hostname\ to\ an\ IP\ via\ socket.getaddrinfo\(\)\ at\ validation\ time\,\ then\ passes\ the\ original\ URL\ to\ httpx\ for\ the\ actual\ request.\ A\ malicious\ domain\ can\ serve\ a\ public\ IP\ during\ validation\ and\ then\ switch\ its\ DNS\ to\ an\ internal\ IP\ \(192.168.x.x\,\ 10.x.x.x\,\ 169.254.x.x\)\ when\ httpx\ makes\ the\ actual\ request.\ Fix:\ After\ DNS\ validation\ passes\,\ pre-resolve\ the\ target\ IP\ once\ and\ use\ it\ directly\ in\ the\ httpx\ request\ by\ replacing\ the\ hostname\ in\ the\ URL\ with\ the\ resolved\ IP\ and\ setting\ a\ Host\ header\,\ so\ no\ second\ DNS\ lookup\ happens.\ Or\ add\ a\ second\ socket.getaddrinfo\ call\ immediately\ before\ the\ httpx\ call\ in\ _action_webhook\ to\ re-validate.\ Read\ the\ file\ first\ to\ understand\ the\ full\ webhook\ action\ flow.\ Run\ make\ lint\ \&\&\ pytest\ -x\ --tb=short.\ Commit:\ security:\ mitigate\ DNS\ rebinding\ TOCTOU\ in\ webhook\ validation
...
PROMPTS_DIR=/home/dotmac/projects/dotmac_erp/.seabone/prompts
