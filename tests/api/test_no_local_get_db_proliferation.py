...
_CANONICAL = {
    ...
    "app/api/deps.py",
    # Shared service-principal bootstrap must resolve the API key's tenant
    # before an organization-scoped session can be created.
    "app/api/service_principal.py",
}
...
# missed the underscored variant, which hid real RLS holes, including
# app/api/finance/fx.py::_get_db, until the 2026-05-16 route-layer audit.
# The route module is now migrated;
