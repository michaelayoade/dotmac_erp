    ...
    with pytest.raises(HTTPException):
        SupplierInvoiceService.create_invoice(
            ...
            SupplierInvoiceInput(
                ...
                due_date=date.today(),
                currency_code="NGN",
                lines=[InvoiceLineInput(description="A", quantity=Decimal("1"), unit_price=Decimal("10"))],
            ),
...
def test_create_credit_note_negative_amounts_and_tax_lines():
    ...
    with (
        patch("app.services.finance.ap.supplier_invoice.SequenceService.get_next_number", return_value="SI-1"),
        patch("app.services.finance.ap.supplier_invoice.SupplierInvoiceService._require_org_match", return_value=None),
        patch("app.services.finance.ap.supplier_invoice.SupplierInvoiceService._require_po_line_org", return_value=None),
        patch("app.services.finance.ap.supplier_invoice.SupplierInvoiceService._require_gr_line_org", return_value=None),
        patch("app.services.finance.ap.supplier_invoice.TaxCalculationService.calculate_line_taxes", return_value=tax_result),
    ):
        invoice = SupplierInvoiceService.create_invoice(
            ...
            SupplierInvoiceInput(
                ...
                due_date=date.today(),
                currency_code="NGN",
                lines=[InvoiceLineInput(description="A", quantity=Decimal("1"), unit_price=Decimal("100"), tax_code_ids=[uuid4()])],
            ),
            created_by_user_id=uuid4(),
        )
...
def test_update_invoice_requires_draft():
    db = MagicMock()
    org_id = uuid4()
    invoice = SimpleNamespace(organization_id=org_id, status=SupplierInvoiceStatus.POSTED)
...
def test_submit_approve_post_void_hold_release_record_payment():
    ...
    submitted = SupplierInvoiceService.submit_invoice(db, org_id, invoice.invoice_id, submitted_by_user_id=uuid4())
    ...
    with pytest.raises(HTTPException):
        SupplierInvoiceService.approve_invoice(db, org_id, invoice.invoice_id, invoice.submitted_by_user_id)

    invoice.submitted_by_user_id = uuid4()
    approved = SupplierInvoiceService.approve_invoice(db, org_id, invoice.invoice_id, approved_by_user_id=uuid4())
    assert approved.status == SupplierInvoiceStatus.APPROVED

    with (
        patch("app.services.finance.ap.ap_posting_adapter.APPostingAdapter.post_invoice") as post_invoice,
        patch("app.services.finance.ap.supplier_invoice.SupplierInvoiceService._update_item_costs_from_invoice") as update_costs,
    ):
        post_invoice.return_value = SimpleNamespace(success=True, journal_entry_id=uuid4(), posting_batch_id=uuid4())
        posted = SupplierInvoiceService.post_invoice(db, org_id, invoice.invoice_id, posted_by_user_id=uuid4())
        assert posted.status == SupplierInvoiceStatus.POSTED

    invoice.status = SupplierInvoiceStatus.DRAFT
    voided = SupplierInvoiceService.void_invoice(db, org_id, invoice.invoice_id, voided_by_user_id=uuid4(), reason="bad")
    assert voided.status == SupplierInvoiceStatus.VOID

    invoice.status = SupplierInvoiceStatus.SUBMITTED
    on_hold = SupplierInvoiceService.put_on_hold(db, org_id, invoice.invoice_id, reason="hold")
    ...
    paid = SupplierInvoiceService.record_payment(db, org_id, invoice.invoice_id, payment_amount=Decimal("40.00"))
