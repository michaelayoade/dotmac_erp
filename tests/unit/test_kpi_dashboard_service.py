...
@pytest.fixture
def scope():
    return DashboardScope(uuid4(), uuid4(), False, ({"id": str(uuid4()), "name": "Engineering", "active": True},), "Africa/Lagos")
...
def test_query_tenant_constrains_source_and_both_joins(db, scope):
    stmt = KPIDashboardService(db)._base_query(scope, DashboardConfig(), date(2026, 9, 15))
...
def test_search_is_literal_and_bound(db, scope):
    stmt = KPIDashboardService(db)._base_query(scope, DashboardConfig(search="a%b_c"), date(2026, 9, 15))
    compiled = stmt.compile(dialect=postgresql.dialect())
...
def test_forbidden_department_fails_before_any_result_query(db, scope):
    with pytest.raises(DashboardAccessError):
        KPIDashboardService(db).dashboard(scope, DashboardConfig(department_ids=(str(uuid4()),)))
...
def test_saved_department_scope_is_rechecked_after_access_changes(db, scope):
    ...
    identifier = str(uuid4())
    db.scalars.return_value.one_or_none.return_value = SimpleNamespace(metadata_={
        PREFERENCE_KEY: {"version": 1, "views": [{"id": identifier, "name": "Revoked", "config": DashboardConfig(department_ids=(revoked_department,)).to_dict()}]},
    })
    with pytest.raises(DashboardAccessError):
...
def test_scope_uses_active_actor_and_current_department_head(db, scope):
    ...
    department = SimpleNamespace(department_id=uuid4(), department_name="Engineering", is_active=True)
    db.scalars.side_effect = [
        MagicMock(**{"one_or_none.return_value": SimpleNamespace(timezone="Africa/Lagos")}),
        MagicMock(**{"one_or_none.return_value": actor}),
        MagicMock(**{"all.return_value": [department]}),
    ]
    result = KPIDashboardService(db).resolve_scope(scope.organization_id, scope.person_id, ["payroll_admin"])
...
def test_no_active_department_head_assignment_denies(db, scope):
    ...
    with pytest.raises(DashboardAccessError):
        KPIDashboardService(db).resolve_scope(scope.organization_id, scope.person_id, [])


def test_dashboard_does_not_invent_missing_actuals_or_average_mixed_units(db, scope):
    counts = {"tracked": 26, "owners": 2, "achieved": 1, "at_risk": 2, "overdue": 3, "reported": 13, "latest_record_update": None}
    db.execute.side_effect = [
        MagicMock(**{"mappings.return_value.one.return_value": counts}),
        MagicMock(**{"mappings.return_value.all.return_value": []}),
        MagicMock(**{"mappings.return_value.all.return_value": [{"actual_value": None}, {"actual_value": Decimal("0")}]}),
    ]
    result = KPIDashboardService(db).dashboard(scope, DashboardConfig(), page=999, now=datetime(2026, 9, 15, tzinfo=timezone.utc))
