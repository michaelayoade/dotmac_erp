...
Phase 1 of the platform adoption ledger: pin the external JSON API contract
so refactors (and later kernel adoption) cannot change it silently. See
``openapi_contract_lib`` for what the manifest covers, why the bare legacy
router aliases are excluded, and why the full OpenAPI document is not
snapshotted wholesale.
