...
def test_reserved_historical_enum_values_have_no_executable_consumer() -> None:
    ...
    for path in sorted((ROOT / "app").rglob("*.py")):
        ...
        if (
            "DOTMAC_CRM" in source
            and relative != "app/models/sync/integration_config.py"
        ):
...
def test_committed_openapi_surface_has_no_retired_crm_contract() -> None:
    manifest = json.loads(_source("tests/architecture/openapi_contract_surface.json"))
    ...
    material_and_operational_routes = [
        route
        for route in routes
