...
def declared_poetry_version(pyproject_text: str) -> str | None:
    """The exact Poetry version that owns lock generation for this repo."""
    version = (
...
def locked_poetry_generator(lock_text: str) -> str | None:
    """Read the Poetry version from the generated lock-file header."""
    match = re.match(
        r"^# This file is automatically @generated by Poetry ([^ ]+) ", lock_text
    )
    return match.group(1) if match else None


POETRY_VERSION = declared_poetry_version(PYPROJECT.read_text()) or ""
...
def test_pyproject_owns_the_exact_poetry_generator_version() -> None:
    declared = declared_poetry_version(PYPROJECT.read_text())

    assert declared is not None and _EXACT_VERSION.fullmatch(declared), (
        "[tool.poetry].requires-poetry must be one exact version"
    )
    assert locked_poetry_generator(LOCK.read_text()) == declared, (
        "poetry.lock must be regenerated by the exact Poetry version declared "
        "in pyproject.toml"
    )
