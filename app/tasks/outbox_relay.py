...
def handle_organization_calendar_changed(db: Session, event: Any) -> None:
    """Settle commands queued before Talk became the calendar transport.

    New calendar consequences are Notification rows delivered by the Talk
    worker. Settling old commands prevents delayed work from reviving the
    retired Nextcloud Calendar projection.
    """
    logger.info("Settling retired calendar projection command %s", event.event_id)
