"""
Invoice Line Tax Model - AR Schema.

Allows multiple tax codes per invoice line for flexible tax handling.
"""
import uuid
...
class InvoiceLineTax(Base):
    """
    Tax applied to an invoice line.

    Allows multiple tax codes per line (e.g., VAT + Stamp Duty).
    ...
    line_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("ar.invoice_line.line_id"),
        nullable=False,
        index=True,
    )
    ...
    # Recoverability tracking (for symmetry with AP and special cases like bad debt relief)
    ...
    recoverable_amount: Mapped[Decimal] = mapped_column(
        ...
        nullable=False,
        default=0,
        comment="Amount eligible for special treatment (e.g., bad debt relief)",
    )
...
# Forward reference
from app.models.ifrs.ar.invoice_line import InvoiceLine  # noqa: E402
