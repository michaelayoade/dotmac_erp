...
class SchedulingPolicy(Base, AuditMixin):
    ...
    scheduling_policy_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        primary_key=True,
        default=uuid.uuid4,
        server_default=text("gen_random_uuid()"),
    ...
    department_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("hr.department.department_id"), nullable=True
    )
...
class ScheduleAuditEvent(Base):
    ...
    __table_args__ = (
        Index("idx_schedule_audit_schedule", "work_schedule_id", "created_at"),
        {"schema": "scheduling"},
    )

    schedule_audit_event_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        primary_key=True,
        default=uuid.uuid4,
        server_default=text("gen_random_uuid()"),
    ...
    action: Mapped[ScheduleAuditAction] = mapped_column(
        Enum(ScheduleAuditAction, name="schedule_audit_action", schema="scheduling"),
        nullable=False,
    )
...
class ScheduleNotificationLog(Base):
    ...
    schedule_notification_log_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        primary_key=True,
        default=uuid.uuid4,
        server_default=text("gen_random_uuid()"),
