...
class TaxJurisdiction(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "organization_id", "jurisdiction_code", name="uq_tax_jurisdiction"
        ),
        {"schema": "tax"},
    )
    ...
    future_tax_rate: Mapped[Decimal | None] = mapped_column(
        Numeric(10, 6), nullable=True
    )
    future_rate_effective_from: Mapped[date | None] = mapped_column(Date, nullable=True)

    # Small business/reduced rate
    has_reduced_rate: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False
    )
    reduced_rate: Mapped[Decimal | None] = mapped_column(Numeric(10, 6), nullable=True)
    reduced_rate_threshold: Mapped[Decimal | None] = mapped_column(
    ...
    filing_due_months: Mapped[int] = mapped_column(
        Numeric(2, 0), nullable=False, default=6
    )
    extension_months: Mapped[int | None] = mapped_column(Numeric(2, 0), nullable=True)

    # Currency
    currency_code: Mapped[str] = mapped_column(String(3), nullable=False)

    # Tax authority
    tax_authority_name: Mapped[str | None] = mapped_column(String(200), nullable=True)
