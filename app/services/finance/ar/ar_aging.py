...
class ARAgingService(ListResponseMixin):
    ...
    def build_aging_report(
        ...
        """Build complete AR aging report response.
        ...
        """
        org_summary = ARAgingService.calculate_organization_aging(
            db=db,
        ...
        if customer_id:
            ...
            customer_summaries = ARAgingService.get_aging_by_customer(
                db=db,
                organization_id=organization_id,
                as_of_date=as_of_date,
            )
        ...
        # When filtered to a single customer, use that customer's data for totals
        # instead of the full org summary
        if customer_id and len(customer_summaries) == 1:
            s = customer_summaries[0]
            totals = {
    ...
    def get_overdue_invoices(
        ...
        invoices = list(db.scalars(stmt.order_by(Invoice.due_date)).all())
