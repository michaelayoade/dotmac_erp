...
class TaxApplicationContextV1:
    ...
    Source scope, document identity, correlation and reversal stay on the exact
    :class:`ERPSourceTaxFactV1` submitted for determination; accepting them a
    second time here would let a valid result be rebound to another document.

    C2 is deliberately jurisdiction-currency only. ``functional_currency_code``
    is required and the outbound adapter refuses a result in any other currency.
    A later FX contract must carry a persisted rate identity, pair, type, event
    date and evidence before foreign-currency rows become admissible; a free
    Decimal rate is not that contract.
    ...
    def __post_init__(self) -> None:
        ...
        currency_code = _require_text(
            self.functional_currency_code, "functional currency code"
        ).upper()
        if len(currency_code) != 3 or not currency_code.isalpha():
            raise TaxAdapterRefusal(
                "functional currency code must be a three-letter alphabetic code"
            )
        object.__setattr__(self, "functional_currency_code", currency_code)
        object.__setattr__(
            self,
            "description",
