...
class ExpenseAdvancesWebMixin:
    @staticmethod
    def cash_advances_list_response(request: Request, auth, db, status: str | None, page: int) -> HTMLResponse:
    ...
    @staticmethod
    def cash_advance_detail_response(request: Request, auth, db, advance_id: str) -> HTMLResponse:
        from app.models.expense.cash_advance import CashAdvanceStatus
        ...
        if advance.status in [CashAdvanceStatus.DISBURSED, CashAdvanceStatus.PARTIALLY_SETTLED]:
            claims = svc.list_claims(org_id, employee_id=advance.employee_id, pagination=PaginationParams(offset=0, limit=20))
            linked_claims = [claim for claim in claims.items if claim.cash_advance_id == advance.advance_id]
    ...
    @staticmethod
    def disburse_cash_advance_response(advance_id: str, bank_account_id: str, payment_mode: str, payment_reference: str | None, auth, db) -> RedirectResponse:
        from app.tasks.expense import post_cash_advance_disbursement

        org_id = coerce_uuid(auth.organization_id)
        svc = ExpenseService(db)
        try:
            advance = svc.disburse_advance(org_id, coerce_uuid(advance_id), payment_reference=payment_reference)
            db.flush()
    ...
    @staticmethod
    def settle_cash_advance_response(advance_id: str, claim_id: str, settlement_amount: str | None, auth, db) -> RedirectResponse:
