...
class ExpenseDashboardChartsMixin:
    def _get_claims_chart_data(self, db: Session, org_id: UUID, start_date: date | None) -> dict[str, Any]:
        return {
    ...
    def _get_monthly_amounts(self, db: Session, org_id: UUID) -> list[dict[str, Any]]:
        ...
        for i in range(5, -1, -1):
            ...
            amounts.append({"month": month_name, "claimed": float(claimed), "paid": float(paid)})
        return amounts
    ...
    def _get_chart_data(self, db: Session, org_id: UUID, start_date: date | None) -> dict[str, Any]:
        return {
            "expense_trend": self._get_expense_trend(db, org_id),
            "category_distribution": self._get_category_distribution(db, org_id, start_date),
            "top_spenders": self._get_top_spenders(db, org_id, start_date),
            "status_breakdown": self._get_status_breakdown(db, org_id, start_date),
            "department_spending": self._get_department_spending(db, org_id, start_date),
        }
    ...
    def _get_expense_trend(self, db: Session, org_id: UUID) -> list[dict[str, Any]]:
        ...
        for i in range(5, -1, -1):
            ...
            trend.append({"month": month_name, "submitted": float(submitted), "reimbursed": float(reimbursed)})
        return trend
    ...
    def _get_category_distribution(
        ...
        results = db.execute(
            select(ExpenseCategory.category_name, func.coalesce(func.sum(ExpenseClaimItem.claimed_amount), 0))
            ...
            .where(and_(*base_filter, ExpenseClaim.status.in_(REPORTABLE_EXPENSE_CLAIM_STATUSES)))
    ...
    def _get_top_spenders(self, db: Session, org_id: UUID, start_date: date | None) -> list[dict[str, Any]]:
        ...
        results = db.execute(
            select(employee_name_col, func.coalesce(func.sum(ExpenseClaim.total_claimed_amount), 0).label("total"))
            .join(Employee, Employee.employee_id == ExpenseClaim.employee_id)
            .join(Person, Person.id == Employee.person_id)
        ...
        return [{"name": row.employee_name, "amount": float(row.total)} for row in results]

    def _get_status_breakdown(self, db: Session, org_id: UUID, start_date: date | None) -> list[dict[str, Any]]:
        base_filter = [ExpenseClaim.organization_id == org_id]
        if start_date:
            base_filter.append(ExpenseClaim.claim_date >= start_date)
        base_filter.append(ExpenseClaim.status.in_(REPORTABLE_EXPENSE_CLAIM_STATUSES))
    ...
    def _get_department_spending(self, db: Session, org_id: UUID, start_date: date | None) -> list[dict[str, Any]]:
        base_filter = [ExpenseClaim.organization_id == org_id]
        if start_date:
            base_filter.append(ExpenseClaim.claim_date >= start_date)
        base_filter.append(ExpenseClaim.status.in_(REPORTABLE_EXPENSE_CLAIM_STATUSES))
        results = db.execute(
            select(Department.department_name, func.coalesce(func.sum(ExpenseClaim.total_claimed_amount), 0))
            .join(Employee, Employee.employee_id == ExpenseClaim.employee_id)
