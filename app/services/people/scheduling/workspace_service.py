...
from app.services.people.scheduling.rules import ScheduleRuleEvaluator, ScheduleRuleResult
...
class ScheduleWorkspaceService:
    ...
    def get_or_create_week(
        ...
        *,
        ...
        self.db.flush()
        self._audit(schedule, actor_person_id, ScheduleAuditAction.CREATED, None, schedule.status.value)
        return schedule
    ...
    def list_assignments(self, organization_id: UUID, schedule_id: UUID) -> list[ShiftSchedule]:
        return list(
            self.db.scalars(
                select(ShiftSchedule)
                .options(joinedload(ShiftSchedule.employee).joinedload(Employee.person), joinedload(ShiftSchedule.shift_type))
                .where(
    ...
    def assign_shift(
        ...
        *,
        ...
        schedule = self._editable_schedule(organization_id, schedule_id, expected_version)
        self.access.assert_department_access(
            organization_id,
        ...
        if employee is None:
            raise ScheduleWorkflowError("Employee is not schedulable in this department")
        ...
        schedule.version += 1
        self.db.flush()
        self._audit(schedule, actor_person_id, action, schedule.status.value, schedule.status.value, {"employee_id": str(employee_id), "shift_date": shift_date.isoformat()})
        return assignment, ScheduleRuleEvaluator(self.db).evaluate(organization_id, schedule)
    ...
    def remove_assignment(
        ...
        *,
        ...
        schedule = self._editable_schedule(organization_id, schedule_id, expected_version)
        self.access.assert_department_access(organization_id, schedule.department_id, actor_employee_id=actor_employee_id, can_view_all=can_view_all)
        assignment = self.db.scalar(select(ShiftSchedule).where(ShiftSchedule.organization_id == organization_id, ShiftSchedule.shift_schedule_id == assignment_id, ShiftSchedule.work_schedule_id == schedule.work_schedule_id))
        if assignment is None:
            raise ScheduleWorkflowError("Assignment not found")
        self.db.delete(assignment)
        schedule.version += 1
        self.db.flush()
        self._audit(schedule, actor_person_id, ScheduleAuditAction.REMOVED, schedule.status.value, schedule.status.value)
        return ScheduleRuleEvaluator(self.db).evaluate(organization_id, schedule)

    def submit(self, organization_id: UUID, schedule_id: UUID, actor_person_id: UUID | None) -> ScheduleRuleResult:
        ...
        if schedule.status not in {ScheduleStatus.DRAFT, ScheduleStatus.REJECTED}:
            raise ScheduleWorkflowError("Only draft or rejected schedules can be submitted")
        ...
        self._audit(schedule, actor_person_id, ScheduleAuditAction.SUBMITTED, previous, schedule.status.value)
        self.db.flush()
        return result

    def approve(self, organization_id: UUID, schedule_id: UUID, actor_person_id: UUID | None) -> None:
        schedule = self.get_workspace(organization_id, schedule_id)
        ...
        if schedule.submitted_by_id and schedule.submitted_by_id == actor_person_id:
            raise ScheduleWorkflowError("Schedule submitter cannot approve their own schedule")
        previous = schedule.status.value
        ...
        schedule.version += 1
        self._sync_assignment_status(schedule)
        self._audit(schedule, actor_person_id, ScheduleAuditAction.APPROVED, previous, schedule.status.value)
        self.db.flush()

    def reject(self, organization_id: UUID, schedule_id: UUID, actor_person_id: UUID | None, reason: str) -> None:
        if not reason.strip():
        ...
        schedule.version += 1
        self._sync_assignment_status(schedule)
        self._audit(schedule, actor_person_id, ScheduleAuditAction.REJECTED, previous, schedule.status.value, reason=reason.strip())
        self.db.flush()

    def publish(self, organization_id: UUID, schedule_id: UUID, actor_person_id: UUID | None) -> int:
        schedule = self.get_workspace(organization_id, schedule_id)
        ...
        schedule.version += 1
        self._sync_assignment_status(schedule)
        self._audit(schedule, actor_person_id, ScheduleAuditAction.PUBLISHED, previous, schedule.status.value)
        ...
        *,
        ...
        existing_draft = self.db.scalar(
            ...
            .where(
                ...
                WorkSchedule.status.in_([ScheduleStatus.DRAFT, ScheduleStatus.REJECTED, ScheduleStatus.SUBMITTED, ScheduleStatus.APPROVED]),
            )
            .order_by(WorkSchedule.revision.desc())
            .limit(1)
        )
        ...
        for assignment in self.list_assignments(organization_id, source.work_schedule_id):
            self.db.add(
                ShiftSchedule(
                    organization_id=organization_id,
        ...
        self.db.flush()
        return amendment
    def evaluation(self, organization_id: UUID, schedule_id: UUID) -> ScheduleRuleResult:
        return ScheduleRuleEvaluator(self.db).evaluate(organization_id, self.get_workspace(organization_id, schedule_id))

    def _editable_schedule(self, organization_id: UUID, schedule_id: UUID, expected_version: int | None) -> WorkSchedule:
        schedule = self.get_workspace(organization_id, schedule_id)
        if expected_version is not None and schedule.version != expected_version:
            raise ScheduleConcurrencyError("Schedule has changed; reload before editing")
        if schedule.status not in {ScheduleStatus.DRAFT, ScheduleStatus.REJECTED}:
    ...
    def _sync_assignment_status(self, schedule: WorkSchedule) -> None:
        for assignment in self.list_assignments(schedule.organization_id, schedule.work_schedule_id):
            assignment.status = schedule.status
            assignment.revision = schedule.revision

    def _audit(self, schedule: WorkSchedule, actor_id: UUID | None, action: ScheduleAuditAction, previous: str | None, new: str | None, metadata: dict | None = None, reason: str | None = None) -> None:
        self.db.add(ScheduleAuditEvent(organization_id=schedule.organization_id, work_schedule_id=schedule.work_schedule_id, actor_id=actor_id, action=action, previous_status=previous, new_status=new, reason=reason, event_metadata=metadata or {}))

    def _notify_published(self, schedule: WorkSchedule, actor_id: UUID | None) -> int:
        notification_service = NotificationService()
        sent = 0
        employee_ids = {assignment.employee_id for assignment in self.list_assignments(schedule.organization_id, schedule.work_schedule_id)}
        employees = list(self.db.scalars(select(Employee).where(Employee.organization_id == schedule.organization_id, Employee.employee_id.in_(employee_ids))).all())
        for employee in employees:
            if not employee.person_id:
                continue
            existing = self.db.scalar(select(ScheduleNotificationLog).where(ScheduleNotificationLog.organization_id == schedule.organization_id, ScheduleNotificationLog.work_schedule_id == schedule.work_schedule_id, ScheduleNotificationLog.revision == schedule.revision, ScheduleNotificationLog.employee_id == employee.employee_id))
            if existing:
                continue
            notification = notification_service.create(
                ...
                title="Schedule Published" if schedule.revision == 1 else "Schedule Updated",
            ...
            self.db.add(ScheduleNotificationLog(organization_id=schedule.organization_id, work_schedule_id=schedule.work_schedule_id, revision=schedule.revision, employee_id=employee.employee_id, notification_id=getattr(notification, "notification_id", None)))
            sent += 1
        return sent
