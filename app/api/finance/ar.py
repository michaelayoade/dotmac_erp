...
from app.services.finance.ar import ecl_service, ECLCalculationInput, GeneralApproachInput
...
class ECLStageBreakdownRead(BaseModel):
    """Stage-level ECL breakdown."""
    gross_carrying_amount: str
    provision: str
    coverage_ratio: str
...
class ECLSummaryRead(BaseModel):
    ...
    total_gross_carrying_amount: str
    total_provision: str
    coverage_ratio: str
    stage_breakdown: dict[str, ECLStageBreakdownRead]
...
def calculate_simplified_ecl(
    ...
    organization_id: UUID = Depends(require_organization_id),
    created_by_user_id: UUID = Query(...),
    auth: dict = Depends(require_tenant_permission("ar:ecl:calculate")),
    db: Session = Depends(get_db),
):
...
def get_ecl_summary(
    organization_id: UUID = Depends(require_organization_id),
    as_of_date: date = Query(...),
    auth: dict = Depends(require_tenant_permission("ar:ecl:read")),
    db: Session = Depends(get_db),
):
    """Get ECL provision summary."""
    summary = ecl_service.get_provision_summary(db, organization_id, as_of_date)
    summary.setdefault("coverage_ratio", "0")
    summary.setdefault("stage_breakdown", {})
    if summary.get("calculation_date"):
        summary["calculation_date"] = date.fromisoformat(summary["calculation_date"])
    summary["as_of_date"] = date.fromisoformat(summary["as_of_date"])
    return summary
