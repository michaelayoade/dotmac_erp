...
from app.schemas.ifrs.banking import (
    # Bank Account
    BankAccountCreate,
    BankAccountRead,
...
from app.services.ifrs.banking import (
    BankAccountInput,
    ReconciliationInput,
...
def get_bank_statement(
    ...
    result = bank_statement_service.get(db, statement_id)
    if not result:
...
def get_bank_statement_with_lines(
    ...
    result = bank_statement_service.get_with_lines(db, statement_id)
    if not result:
        raise HTTPException(status_code=404, detail="Statement not found")
    if result.organization_id != organization_id:
        raise HTTPException(status_code=404, detail="Statement not found")
...
def get_unmatched_statement_lines(
    ...
    statement = bank_statement_service.get(db, statement_id)
