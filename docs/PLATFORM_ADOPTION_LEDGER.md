...
| sub↔erp sync (AR) | Pull-based per the checked-in contract ("Sub owns ISP billing facts; ERP owns accounting; no second push path"): watermarked incremental client w/ retries + circuit cooldown; cents-quantize; content-hash change detection; reverse-and-repost on posted-invoice changes; webhook ingress (HMAC, delivery-id dedup) feeding the SAME sync service; daily + full reconciliation tasks | reuse (contract + client pattern) / product-owned (services) | Exemplary. The integration-audit "shared-client keystone" lives here |
