...
def _find_bank_line(
    ...
    *,
    ...
    base_conditions = [
        ...
        BankStatementLine.amount == amount,
    ]
    ...
    candidates = list(
        db.execute(
            ...
            .order_by(
                # Prefer closest date (ERPNext posting_date should match bank tx date)
    ...
    # If multiple candidates exist, consume the first. We rely on the claim-side
    # posting to make a one-to-one bank match possible. Ambiguity is expected
        ...
        for claim in claims:
            ...
            if not posting.success or not posting.journal_entry_id:
                ...
                stats = Stats(
                    **{
                        **stats.__dict__,
                        "post_failed": stats.post_failed + 1,
                    }
                )
                continue
            ...
            # Find the bank-side line in the reimbursement journal and match it.
            ...
            if not bank_jel:
                ...
                stats = Stats(
                    **{
                        **stats.__dict__,
                        "match_failed": stats.match_failed + 1,
                    }
                )
                continue
            ...
            if pending >= 50:
                ...
                pending = 0
                print(
                    f"progress scanned={stats.scanned} posted={stats.posted} matched={stats.matched} "
                    f"missing_bank_line={stats.missing_bank_line} post_failed={stats.post_failed} match_failed={stats.match_failed}"
                )
