...
def process_file(filepath: Path) -> tuple[bool, int]:
    ...
    while i < len(lines):
        ...
        if m and "db.scalars" not in line:
            ...
            result_lines.append(
                f"{indent}{prefix}{db_var}.scalars({select_part}).first(){suffix}"
            )
            count += 1
            i += 1
            continue
        ...
        if m and "db.scalars" not in line:
            ...
            if has_list:
                result_lines.append(
                    f"{indent}{prefix}{db_var}.scalars({select_part}).all(){suffix}"
                )
            else:
                result_lines.append(
                    f"{indent}{prefix}list({db_var}.scalars({select_part}).all()){suffix}"
                )
            count += 1
            i += 1
            continue
        ...
        if m and "db.scalar" not in line:
            ...
            db_var = find_db_var_in_context(lines, i)
            result_lines.append(
                f"{indent}{prefix}{db_var}.scalar({select_part}){suffix}"
            )
            count += 1
            i += 1
            continue
        ...
        if stripped in (".first()", ".all()", ".scalar()") or stripped in (
            ").first()",
            ").all()",
            ").scalar()",
        ):
            ...
            if start_idx is not None:
                ...
                if method == ".first()" or method == ".all()":
                    ...
                    indent_t = re.match(r"(\s*)", line).group(1)
                    if stripped == ".scalar()" or stripped == ").scalar()":
                        lines[i] = f"{indent_t})"
...
def main():
    ...
    for py_file in sorted(app_dir.rglob("*.py")):
        ...
        if (
            ".query(" in text
            and "import" not in text.split(".query(")[0].split("\n")[-1]
        ):
            needs_work = True
        if "select(" in text:
            ...
            for pattern in [".first()", ".all()", ".scalar()"]:
                if (
                    pattern in text
                    and "db.scalars" not in text.split(pattern)[0].split("\n")[-1]
                ):
                    needs_work = True
                    break
        if needs_work:
