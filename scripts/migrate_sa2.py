...
def migrate_file(filepath: Path) -> tuple[bool, int]:
    ...
    for m in re.finditer(r'Query\(\[([^\]]+)\],\s*session\s*=\s*(\w+(?:\.\w+)*)\)', content):
            ...
            if m:
                ...
                if 'select' not in imports:
                    content = content.replace(
                        m.group(0),
                        f"{m.group(1)}select, {imports}" if not imports.startswith('select') else m.group(0),
                    )
                    count += 1
            else:
...
def main():
    ...
    print("  2. .all() / .first() / .scalar() calls need db.scalars()/db.execute() wrapping")
