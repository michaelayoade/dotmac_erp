...
# Import paths
IMPORT_PATH = Path(
    "/Users/michaelayoade/Downloads/Projects/dotmac_erp/books_backup/import_ready"
)
...
class BulkImporter:
    ...
    def _setup_default_accounts(self):
        ...
        accounts = (
            self.db.query(Account).filter(Account.organization_id == self.org_id).all()
        )
        for acc in accounts:
    ...
    def _build_customer_cache(self):
        ...
        customers = (
            ...
            .filter(Customer.organization_id == self.org_id)
            .all()
        )
        for c in customers:
    ...
    def _build_supplier_cache(self):
        ...
        suppliers = (
            ...
            .filter(Supplier.organization_id == self.org_id)
            .all()
        )
        for s in suppliers:
    ...
    def _ensure_default_item_category(self) -> UUID | None:
        ...
        if not all(
            [
                ...
                self._cogs_account_id,
                self._revenue_account_id,
    ...
    def import_chart_of_accounts(self) -> dict[str, Any]:
        ...
        self.log(
            f"Imported: {stats['imported']}, Skipped: {stats['skipped']}, Errors: {len(stats['errors'])}"
        )
        return stats
        ...
        with open(filepath, encoding="utf-8", errors="ignore") as f:
            ...
            for row in reader:
                try:
                    ...
                    email = (
                        row.get("EmailID", "").strip() or row.get("Email", "").strip()
                    )
                    phone = (
                        row.get("Phone", "").strip()
                        or row.get("MobilePhone", "").strip()
                    )
                    primary_contact = {}
                    if email:
                    ...
                    if row.get("First Name", "").strip():
                        primary_contact["first_name"] = row.get(
                            "First Name", ""
                        ).strip()
                    ...
                    if row.get("Billing Country", "").strip():
                        billing_address["country"] = row.get(
                            "Billing Country", ""
                        ).strip()
                    ...
                    if not self.dry_run:
                        customer_code = self._generate_code(
                            "CUS", display_name, customer_index
                        )
                        ...
                        customer = Customer(
                            ...
                            is_active=row.get("Status", "Active").strip().lower()
                            == "active",
                        )
        ...
        with open(filepath, encoding="utf-8", errors="ignore") as f:
            ...
            for row in reader:
                try:
                    ...
                    if not self.dry_run:
                        supplier_code = self._generate_code(
                            "VEN", vendor_name, supplier_index
                        )
                        ...
                        supplier = Supplier(
                            ...
                            trading_name=row.get("Company Name", "").strip()[:255]
                            or None,
                            currency_code=currency,
    ...
    def import_items(self) -> dict[str, Any]:
        ...
        if not category_id and not self.dry_run:
            self.log(
                "ERROR: Cannot create default item category. Missing required accounts."
            )
        ...
        with open(filepath, encoding="utf-8", errors="ignore") as f:
            ...
            for row in reader:
                try:
                    item_name = (
                        row.get("Item Name", "").strip() or row.get("Name", "").strip()
                    )
                    if not item_name:
                        continue
                    ...
                    if not self.dry_run:
                        item_code = (
                            row.get("SKU", "").strip()
                            or row.get("Item Code", "").strip()
                        )
                        if not item_code:
                            item_code = self._generate_code(
                                "ITM", item_name, item_index
                            )
    ...
    def import_invoices(self) -> dict[str, Any]:
        ...
        self.log(
            f"Imported: {stats['imported']}, Skipped: {stats['skipped']}, No Customer: {stats['no_customer']}, Errors: {len(stats['errors'])}"
        )
        return stats
    ...
    def import_bills(self) -> dict[str, Any]:
        ...
        self.log(
            f"Imported: {stats['imported']}, Skipped: {stats['skipped']}, No Supplier: {stats['no_supplier']}, Errors: {len(stats['errors'])}"
        )
        return stats
                ...
                try:
                    ...
                    if not self.dry_run:
                        payment = CustomerPayment(
                            ...
                            reference=row.get("Reference Number", "").strip()[:100]
                            or None,
                            description=row.get("Description", "").strip() or None,
    ...
    def import_journals(self) -> dict[str, Any]:
        ...
        with open(filepath, encoding="utf-8", errors="ignore") as f:
            ...
            for row in reader:
                ...
                if journal_number not in journal_data:
                    journal_data[journal_number] = {"header": row, "lines": []}
            ...
            try:
                ...
                if not self.dry_run:
                    journal = JournalEntry(
                        ...
                        description=header.get("Notes", "").strip()[:500]
                        or f"Imported Journal {journal_number}",
                        reference=header.get("Reference Number", "").strip()[:100]
                        or None,
                        currency_code=currency,
        ...
        self.log(
            f"Imported: {stats['imported']}, Skipped: {stats['skipped']}, No Period: {stats['no_period']}, Errors: {len(stats['errors'])}"
        )
        return stats
    ...
    def _map_invoice_status(
        self, zoho_status: str, balance: Decimal, total: Decimal
    ) -> InvoiceStatus:
    ...
    def run_all(self):
        ...
        for entity, stats in results.items():
            ...
            print(
                f"  {entity:<20} Imported: {imported:>6}, Skipped: {skipped:>6}, Errors: {errors:>4}{extra}"
            )

        print("-" * 60)
        print(
            f"  {'TOTAL':<20} Imported: {total_imported:>6}, Skipped: {total_skipped:>6}, Errors: {total_errors:>4}"
        )
    ...
    try:
        ...
        if args.entity:
            ...
            if hasattr(importer, method_name):
                ...
                print(
                    "Available: accounts, customers, vendors, items, invoices, bills, customer_payments, vendor_payments, journals"
                )
                sys.exit(1)
        else:
            importer.run_all()
