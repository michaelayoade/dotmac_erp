"""Temporary implementation driver, excluded from every fix PR.

Only ordinary fast-forward pushes to explicit fix branches are allowed.
Production data, deployment descriptors, and main are never mutated here.
"""
...
def run(*args, cwd=ROOT, check=True, env=None):
    result = subprocess.run(
        args, cwd=cwd, check=False, text=True, capture_output=True, env=env
    )
    if result.stdout:
...
def main():
    ...
    for recipe_path in sorted(RECIPES.glob("*.py")):
        ...
        start = recipe.get("START", BASE)
        existing = run("git", "ls-remote", "--heads", "origin", f"refs/heads/{branch}").stdout.strip()
        existing_sha = existing.split()[0] if existing else None
        if existing_sha and existing_sha != start:
            print("EXISTING_BRANCH " + branch, flush=True)
            continue
        if start != BASE:
            run("git", "fetch", "--depth=1", "origin", start)
        work = ROOT.parent / ("review-" + recipe_path.stem)
        run("git", "worktree", "add", "--detach", str(work), start)
        changed = set()
        ...
        try:
            ...
            run("git", "switch", "-c", "review-" + recipe_path.stem, cwd=work)
            ...
            report = {"branch": branch, "sha": sha, "files": files, "focused_test_exit": tests.returncode}
            reports.append(report)
            print("FIX_RESULT " + json.dumps(report), flush=True)
            if tests.returncode:
    ...
    if os.environ.get("GITHUB_STEP_SUMMARY"):
        with open(os.environ["GITHUB_STEP_SUMMARY"], "a") as stream:
            stream.write("## ERP log-review verification\n\n```json\n" + json.dumps(reports, indent=2) + "\n```\n")
    if failures:
