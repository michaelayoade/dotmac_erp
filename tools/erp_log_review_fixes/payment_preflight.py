"""Payment prerequisite fix, applied independently from reviewed main."""

BRANCH = "fix/erp-payment-bank-preflight"
TITLE = "fix(sync): validate payment bank prerequisites before receipt mutation"
TESTS = ["tests/services/test_payment_bank_preflight.py", "tests/services/test_dotmac_sub_payment_reversal.py"]


def apply(change, write, rewrite):
    bank = "app/services/dotmac_sub/sync/_bank_mapping.py"
    change(bank, "from app.models.finance.banking.bank_account import BankAccount, BankAccountStatus", "from app.models.finance.banking.bank_account import BankAccount, BankAccountStatus\nfrom app.models.finance.gl.account import Account")
    change(bank, "class BankMappingMixin:", '''class MissingPaymentBankMappingError(ValueError):
    ...
    def __init__(self, payment_id: str, channel_id: str | None, currency_code: str) -> None:
...
class BankMappingMixin:''')
    rewrite(bank, "_get_default_bank_account", lambda _: '''    def _get_default_bank_account(self, currency_code: str) -> UUID | None:
...
''')
    rewrite(bank, "_get_bank_account_for_channel", lambda _: '''    def _get_bank_account_for_channel(
...
''')
    rewrite(bank, "_match_bank_account", lambda _: '''    def _match_bank_account(self, erp_fragment: str, currency_code: str) -> UUID | None:
        ...
        return None
''')
    payments = "app/services/dotmac_sub/sync/_payments.py"
    change(payments, "from ._progress import WatermarkProgress", "from ._bank_mapping import MissingPaymentBankMappingError\nfrom ._progress import WatermarkProgress")
    change(payments, '''                self._ensure_synced_payment_posted(payment, created_by_user_id)
                self._ensure_wht_terminal_consequence(''', '''                if (
    ...
    change(payments, '''        bank_account_id = self._get_bank_account_for_channel(
            pay.payment_channel_id, currency_code
        )
        method =''', '''        bank_account_id = self._get_bank_account_for_channel(
            pay.payment_channel_id, currency_code
        )
        ...
        method =''')
    change(payments, '''                    logger.exception("Error syncing payment %s", pay.id)
                    progress.record_failure(row_updated_at)''', '''                    if isinstance(e, MissingPaymentBankMappingError):
                        logger.warning(
    ...
    change("tests/services/test_dotmac_sub_payment_reversal.py", "    payment = SimpleNamespace(payment_id=uuid.uuid4())", '''    payment = SimpleNamespace(
    ...
    write("tests/services/test_payment_bank_preflight.py", '''"""Do not mutate accounting when bank prerequisites are absent or incompatible."""
...
def test_unchanged_unposted_receipt_can_use_repaired_mapping():
    ...
    local = SimpleNamespace(
        journal_entry_id=None, gross_amount=Decimal("100"),
        bank_account_id=None, currency_code="NGN",
    )
    bank_id = uuid4()
...
def test_unchanged_missing_mapping_does_not_claim_posting_success():
    ...
    with pytest.raises(MissingPaymentBankMappingError):
        service._sync_single_payment(source(), SyncResult(success=True, entity_type="payments"), None, True)
...
def test_zero_value_receipt_does_not_require_bank_mapping():
    service = harness(changed=False)
    local = SimpleNamespace(journal_entry_id=None, gross_amount=Decimal("0"), bank_account_id=None)
    service._find_local_payment.return_value = local
    service._sync_single_payment(source("0"), SyncResult(success=True, entity_type="payments"), None, True)
