...
def upgrade() -> None:
    ...
    op.create_table(
        'competency',
        sa.Column('competency_id', postgresql.UUID(as_uuid=True), server_default=sa.text('gen_random_uuid()'), nullable=False),
        ...
        sa.Column('category', postgresql.ENUM('core', 'functional', 'leadership', 'behavioral', name='competency_category', schema='hr', create_type=False), nullable=False),
    ...
    op.create_index('idx_competency_org', 'competency', ['organization_id'], schema='hr')
    op.create_index('idx_competency_code', 'competency', ['competency_code'], schema='hr')

    # Create job_description table
    op.create_table(
        'job_description',
        sa.Column('job_description_id', postgresql.UUID(as_uuid=True), server_default=sa.text('gen_random_uuid()'), nullable=False),
        sa.Column('organization_id', postgresql.UUID(as_uuid=True), nullable=False),
        ...
        sa.Column('status', postgresql.ENUM('draft', 'active', 'under_review', 'archived', name='job_description_status', schema='hr', create_type=False), nullable=False),
    ...
    op.create_index('idx_job_description_org', 'job_description', ['organization_id'], schema='hr')
    op.create_index('idx_job_description_code', 'job_description', ['jd_code'], schema='hr')
    op.create_index('idx_job_description_designation', 'job_description', ['designation_id'], schema='hr')
    op.create_index('idx_job_description_department', 'job_description', ['department_id'], schema='hr')

    # Create job_description_competency junction table
    op.create_table(
        'job_description_competency',
        sa.Column('jd_competency_id', postgresql.UUID(as_uuid=True), server_default=sa.text('gen_random_uuid()'), nullable=False),
        ...
        sa.Column('created_by', postgresql.UUID(as_uuid=True), nullable=True),
        sa.Column('updated_by', postgresql.UUID(as_uuid=True), nullable=True),
        sa.ForeignKeyConstraint(['job_description_id'], ['hr.job_description.job_description_id']),
        sa.ForeignKeyConstraint(['competency_id'], ['hr.competency.competency_id']),
        sa.PrimaryKeyConstraint('jd_competency_id'),
        schema='hr'
    )
    op.create_index('idx_jd_competency_jd', 'job_description_competency', ['job_description_id'], schema='hr')
    op.create_index('idx_jd_competency_comp', 'job_description_competency', ['competency_id'], schema='hr')


def downgrade() -> None:
    op.drop_index('idx_jd_competency_comp', table_name='job_description_competency', schema='hr')
    op.drop_index('idx_jd_competency_jd', table_name='job_description_competency', schema='hr')
    op.drop_table('job_description_competency', schema='hr')

    op.drop_index('idx_job_description_department', table_name='job_description', schema='hr')
    op.drop_index('idx_job_description_designation', table_name='job_description', schema='hr')
