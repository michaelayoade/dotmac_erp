...
def upgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE sync.source_correlation RENAME COLUMN crm_id TO source_reference"
    )
    op.execute(
        "ALTER TABLE sync.source_correlation RENAME COLUMN crm_status TO source_status"
    )
    op.execute(
        "ALTER TABLE sync.source_correlation RENAME COLUMN crm_data TO source_payload"
    )
    op.execute(
        "ALTER TABLE sync.source_correlation "
    ...
    op.execute("ALTER TYPE sync.crm_sync_status RENAME TO source_correlation_status")
    op.execute(
        "ALTER TABLE sync.source_correlation ADD COLUMN source_application VARCHAR(50)"
    )
    op.execute(
    ...
    op.execute(
        "ALTER TABLE sync.source_correlation DROP CONSTRAINT uq_crm_sync_org_type_id"
    )
    op.execute(
        "ALTER TABLE sync.source_correlation "
    ...
    op.execute(
        "ALTER INDEX sync.idx_crm_sync_local RENAME TO idx_source_correlation_local"
    )
    op.execute(
        "ALTER INDEX sync.idx_crm_sync_status RENAME TO idx_source_correlation_status"
    )
    op.execute(
    ...
    op.execute("ALTER TABLE sync.source_correlation ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE sync.source_correlation FORCE ROW LEVEL SECURITY")
    op.execute(
        """
    ...
    op.execute(
        "ALTER TABLE expense.expense_claim DROP CONSTRAINT uq_expense_claim_org_crm_id"
    )
    op.execute(
