...
def upgrade() -> None:
    op.execute(
        "ALTER TYPE pm.project_type ADD VALUE IF NOT EXISTS 'FIBER_OPTICS_INSTALLATION'"
    )
    op.execute(
        "ALTER TYPE pm.project_type ADD VALUE IF NOT EXISTS 'AIR_FIBER_INSTALLATION'"
    )
    op.execute("ALTER TYPE pm.project_type ADD VALUE IF NOT EXISTS 'CABLE_RERUN'")
    op.execute(
        "ALTER TYPE pm.project_type ADD VALUE IF NOT EXISTS 'FIBER_OPTICS_RELOCATION'"
    )
    op.execute(
        "ALTER TYPE pm.project_type ADD VALUE IF NOT EXISTS 'AIR_FIBER_RELOCATION'"
    )
