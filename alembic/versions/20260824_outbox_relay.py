"""Host the kernel outbox relay contract in ERP's own lineage.

ERP cannot run or stamp the kernel root, so every effect a composed module
...
## This is NOT ERP's business-event outbox

``platform.event_outbox`` remains the authority for ERP business events, with
its own aggregate vocabulary, idempotency key, headers and handler checkpoints.
The tables created here carry MODULE events only — what
``dotmac_kernel.messaging`` enqueues and drains for composed modules such as
``dotmac-approvals`` and ``dotmac-durable-timers``. Two mechanisms, two
vocabularies, one authority each; neither reads the other's rows, and no ERP
consequence moves onto this relay in this slice.
...
``platform_outbox_dispatcher`` with a ``DO`` block. ERP does not: ``CREATE ROLE``
needs privileges an ordinary ``alembic upgrade`` must never hold, and
``20260814_database_roles`` already established that creation belongs to the
explicitly elevated ``scripts/bootstrap_database_roles.py``. So this revision
fails closed on a cluster whose dispatcher roles are absent or wrong-shaped,
and the bootstrap script grew both roles in the same change.
...
Revision ID: 20260824_outbox_relay
Revises: 20260824_party_person_projection
...
revision = "20260824_outbox_relay"
down_revision = "20260824_party_person_projection"
...
#: Point-in-time copy of `app.migration_database_roles.RELAY_DISPATCHER_CONTRACT`,
#: as `(rolbypassrls, rolsuper)`. Copied rather than imported for the reason
#: `20260814_database_roles` states: a migration is a snapshot of an accepted
#: decision, and importing a mutable runtime value would let a later edit change
#: what an applied revision meant. The architecture test pins the two together.
DISPATCHER_CONTRACT: dict[str, tuple[bool, bool]] = {
    TENANT_DISPATCHER: (False, False),
    PLATFORM_DISPATCHER: (False, False),
}
...
def _assert_dispatcher_roles_exist() -> None:
    ...
    for role, expected in DISPATCHER_CONTRACT.items():
        ...
        if actual is None:
            ...
            problems.append(
                f"{role} has (rolbypassrls, rolsuper)={actual!r}, "
    ...
    if problems:
        raise RuntimeError(
            ...
            + "; ".join(problems)
            + ". Run scripts/bootstrap_database_roles.py with elevated "
