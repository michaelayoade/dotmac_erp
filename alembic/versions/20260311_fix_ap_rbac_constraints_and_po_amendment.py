...
def _add_unique(conn, name: str, table: str, columns: str,
...
def _add_index(conn, name: str, table: str, columns: str,
               schema: str = "public", *, where: str | None = None,
...
def upgrade() -> None:
    ...
    _add_unique(
        conn, "uq_role_permissions_role_permission",
    ...
    _add_unique(
        conn, "uq_person_roles_person_role",
    ...
    _add_unique(
        conn, "uq_supplier_code",
        "supplier", "organization_id, supplier_code", "ap",
    )
    _add_unique(
        conn, "uq_po_number",
        "purchase_order", "organization_id, po_number", "ap",
    )
    _add_unique(
        conn, "uq_po_line",
        "purchase_order_line", "po_id, line_number", "ap",
    )
    _add_unique(
        conn, "uq_supplier_invoice",
        "supplier_invoice", "organization_id, invoice_number", "ap",
    )
    _add_unique(
        conn, "uq_supplier_invoice_line",
        "supplier_invoice_line", "invoice_id, line_number", "ap",
    )
    _add_unique(
        conn, "uq_line_tax_code",
        "supplier_invoice_line_tax", "line_id, tax_code_id", "ap",
    )
    _add_unique(
        conn, "uq_supplier_payment",
        "supplier_payment", "organization_id, payment_number", "ap",
    )
    _add_unique(
        conn, "uq_ap_allocation",
        "payment_allocation", "payment_id, invoice_id", "ap",
    )
    _add_unique(
        conn, "uq_receipt_number",
        "goods_receipt", "organization_id, receipt_number", "ap",
    )
    _add_unique(
        conn, "uq_receipt_line",
        "goods_receipt_line", "receipt_id, line_number", "ap",
    )
    _add_unique(
        conn, "uq_payment_batch",
        "payment_batch", "organization_id, batch_number", "ap",
    )
    _add_unique(
        conn, "uq_ap_aging",
        "ap_aging_snapshot",
    ...
    _add_index(
        conn, "idx_supplier_org",
        "supplier", "organization_id, is_active", "ap",
    )
    _add_index(
        conn, "idx_po_supplier",
        "purchase_order", "supplier_id", "ap",
    )
    _add_index(
        conn, "idx_po_status",
        "purchase_order", "organization_id, status", "ap",
    )
    _add_index(
        conn, "idx_supplier_invoice_supplier",
        "supplier_invoice", "supplier_id", "ap",
    )
    _add_index(
        conn, "idx_supplier_invoice_status",
        "supplier_invoice", "organization_id, status", "ap",
    )
    _add_index(
        conn, "idx_supplier_invoice_due_date",
        "supplier_invoice", "organization_id, due_date", "ap",
        where="status NOT IN ('PAID', 'VOID')",
    )
    _add_index(
        conn, "idx_supplier_payment_supplier",
        "supplier_payment", "supplier_id", "ap",
    )
    _add_index(
        conn, "idx_receipt_po",
        "goods_receipt", "po_id", "ap",
    )
    ...
    if not _has_column(conn, "purchase_order", "is_amendment", "ap"):
        op.add_column(
            "purchase_order",
            sa.Column(
                "is_amendment", sa.Boolean(), nullable=False,
    ...
    if not _has_column(conn, "purchase_order", "original_po_id", "ap"):
        op.add_column(
            "purchase_order",
            sa.Column(
                "original_po_id", postgresql.UUID(as_uuid=True),
    ...
    if not _has_column(conn, "purchase_order", "amendment_version", "ap"):
        op.add_column(
            "purchase_order",
            sa.Column(
                "amendment_version", sa.Integer(), nullable=False,
    ...
    if not _has_column(conn, "purchase_order", "amendment_reason", "ap"):
        op.add_column(
            "purchase_order",
            sa.Column(
                "amendment_reason", sa.Text(), nullable=True,
    ...
    if not _has_column(conn, "purchase_order", "variation_id", "ap"):
        op.add_column(
            "purchase_order",
            sa.Column(
                "variation_id", sa.String(36), nullable=True,
    ...
    _add_index(
        conn, "idx_po_original_po",
        "purchase_order", "original_po_id", "ap",
        where="original_po_id IS NOT NULL",
    )
    _add_index(
        conn, "uq_po_variation_id",
        "purchase_order", "organization_id, variation_id", "ap",
