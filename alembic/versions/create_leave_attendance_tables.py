...
def upgrade() -> None:
    ...
    op.create_table(
        "leave_type",
        sa.Column("leave_type_id", postgresql.UUID(as_uuid=True), server_default=sa.text("gen_random_uuid()"), nullable=False),
        ...
        sa.Column("allocation_policy", postgresql.ENUM("ANNUAL", "MONTHLY", "EARNED", "UNLIMITED", name="leave_type_policy", create_type=False), nullable=False, server_default="ANNUAL"),
        sa.Column("max_days_per_year", sa.Numeric(5, 1), nullable=True),
        sa.Column("max_continuous_days", sa.Integer, nullable=True),
        sa.Column("allow_carry_forward", sa.Boolean, nullable=False, server_default="false"),
        ...
        sa.Column("allow_encashment", sa.Boolean, nullable=False, server_default="false"),
        ...
        sa.Column("is_compensatory", sa.Boolean, nullable=False, server_default="false"),
        sa.Column("include_holidays", sa.Boolean, nullable=False, server_default="false"),
        ...
        sa.UniqueConstraint("organization_id", "leave_type_code", name="uq_leave_type_org_code"),
        schema="leave",
    )
    op.create_index("idx_leave_type_org", "leave_type", ["organization_id"], schema="leave")
    op.create_index("idx_leave_type_active", "leave_type", ["organization_id", "is_active"], schema="leave")
    ...
    op.create_table(
        "holiday_list",
        sa.Column("holiday_list_id", postgresql.UUID(as_uuid=True), server_default=sa.text("gen_random_uuid()"), nullable=False),
        sa.Column("organization_id", postgresql.UUID(as_uuid=True), nullable=False),
        ...
        sa.Column("weekly_off", sa.String(50), nullable=False, server_default="Saturday,Sunday"),
        ...
        sa.ForeignKeyConstraint(["created_by_id"], ["people.id"]),
        sa.ForeignKeyConstraint(["updated_by_id"], ["people.id"]),
        sa.UniqueConstraint("organization_id", "list_code", name="uq_holiday_list_org_code"),
        schema="leave",
    )
    op.create_index("idx_holiday_list_org", "holiday_list", ["organization_id"], schema="leave")
    op.create_index("idx_holiday_list_year", "holiday_list", ["organization_id", "year"], schema="leave")
    ...
    op.create_table(
        "holiday",
        sa.Column("holiday_id", postgresql.UUID(as_uuid=True), server_default=sa.text("gen_random_uuid()"), nullable=False),
        ...
        sa.Column("is_public_holiday", sa.Boolean, nullable=False, server_default="true"),
        sa.Column("is_optional", sa.Boolean, nullable=False, server_default="false"),
        sa.PrimaryKeyConstraint("holiday_id"),
        sa.ForeignKeyConstraint(["holiday_list_id"], ["leave.holiday_list.holiday_list_id"], ondelete="CASCADE"),
        sa.UniqueConstraint("holiday_list_id", "holiday_date", name="uq_holiday_list_date"),
        schema="leave",
    )
    op.create_index("idx_holiday_date", "holiday", ["holiday_list_id", "holiday_date"], schema="leave")
    ...
    op.create_table(
        "leave_allocation",
        sa.Column("allocation_id", postgresql.UUID(as_uuid=True), server_default=sa.text("gen_random_uuid()"), nullable=False),
        sa.Column("organization_id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column("employee_id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column("leave_type_id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column("from_date", sa.Date, nullable=False),
        sa.Column("to_date", sa.Date, nullable=False),
        sa.Column("new_leaves_allocated", sa.Numeric(5, 1), nullable=False, server_default="0"),
        sa.Column("carry_forward_leaves", sa.Numeric(5, 1), nullable=False, server_default="0"),
        ...
        sa.Column("leaves_encashed", sa.Numeric(5, 1), nullable=False, server_default="0"),
        sa.Column("leaves_expired", sa.Numeric(5, 1), nullable=False, server_default="0"),
        ...
        sa.ForeignKeyConstraint(["created_by_id"], ["people.id"]),
        sa.ForeignKeyConstraint(["updated_by_id"], ["people.id"]),
        sa.UniqueConstraint("employee_id", "leave_type_id", "from_date", name="uq_leave_allocation_emp_type_period"),
        schema="leave",
    )
    op.create_index("idx_leave_allocation_org", "leave_allocation", ["organization_id"], schema="leave")
    op.create_index("idx_leave_allocation_employee", "leave_allocation", ["employee_id", "from_date"], schema="leave")
    op.create_index("idx_leave_allocation_type", "leave_allocation", ["leave_type_id", "from_date"], schema="leave")
    ...
    op.create_table(
        ...
        sa.Column("status", postgresql.ENUM("DRAFT", "SUBMITTED", "APPROVED", "REJECTED", "CANCELLED", name="leave_application_status", create_type=False), nullable=False, server_default="DRAFT"),
        sa.Column("leave_approver_id", postgresql.UUID(as_uuid=True), nullable=True),
        sa.Column("approved_by_id", postgresql.UUID(as_uuid=True), nullable=True),
        sa.Column("approved_at", sa.DateTime(timezone=True), nullable=True),
        sa.Column("rejection_reason", sa.Text, nullable=True),
        sa.Column("is_posted_to_payroll", sa.Boolean, nullable=False, server_default="false"),
    ...
    op.create_index("idx_leave_app_org", "leave_application", ["organization_id"], schema="leave")
    op.create_index("idx_leave_app_employee", "leave_application", ["employee_id", "from_date"], schema="leave")
    op.create_index("idx_leave_app_status", "leave_application", ["organization_id", "status"], schema="leave")
    op.create_index("idx_leave_app_dates", "leave_application", ["organization_id", "from_date", "to_date"], schema="leave")
    ...
    op.create_table(
        "shift_type",
        sa.Column("shift_type_id", postgresql.UUID(as_uuid=True), server_default=sa.text("gen_random_uuid()"), nullable=False),
        sa.Column("organization_id", postgresql.UUID(as_uuid=True), nullable=False),
        ...
        sa.Column("late_entry_grace_period", sa.Integer, nullable=False, server_default="0"),
        sa.Column("early_exit_grace_period", sa.Integer, nullable=False, server_default="0"),
        ...
        sa.Column("enable_overtime", sa.Boolean, nullable=False, server_default="false"),
        ...
        sa.ForeignKeyConstraint(["created_by_id"], ["people.id"]),
        sa.ForeignKeyConstraint(["updated_by_id"], ["people.id"]),
        sa.UniqueConstraint("organization_id", "shift_code", name="uq_shift_type_org_code"),
        schema="attendance",
    )
    op.create_index("idx_shift_type_org", "shift_type", ["organization_id"], schema="attendance")
    op.create_index("idx_shift_type_active", "shift_type", ["organization_id", "is_active"], schema="attendance")
    ...
    op.create_table(
        "attendance",
        sa.Column("attendance_id", postgresql.UUID(as_uuid=True), server_default=sa.text("gen_random_uuid()"), nullable=False),
        sa.Column("organization_id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column("employee_id", postgresql.UUID(as_uuid=True), nullable=False),
        ...
        sa.Column("status", postgresql.ENUM("PRESENT", "ABSENT", "HALF_DAY", "ON_LEAVE", "HOLIDAY", "WORK_FROM_HOME", name="attendance_status", create_type=False), nullable=False),
        ...
        sa.ForeignKeyConstraint(["organization_id"], ["core_org.organization.organization_id"]),
        sa.ForeignKeyConstraint(["employee_id"], ["hr.employee.employee_id"]),
        sa.ForeignKeyConstraint(["shift_type_id"], ["attendance.shift_type.shift_type_id"]),
        sa.ForeignKeyConstraint(["leave_application_id"], ["leave.leave_application.application_id"]),
        sa.ForeignKeyConstraint(["created_by_id"], ["people.id"]),
        sa.ForeignKeyConstraint(["updated_by_id"], ["people.id"]),
        sa.UniqueConstraint("employee_id", "attendance_date", name="uq_attendance_emp_date"),
        schema="attendance",
    )
    op.create_index("idx_attendance_org", "attendance", ["organization_id"], schema="attendance")
    op.create_index("idx_attendance_date", "attendance", ["organization_id", "attendance_date"], schema="attendance")
    op.create_index("idx_attendance_employee", "attendance", ["employee_id", "attendance_date"], schema="attendance")
    op.create_index("idx_attendance_status", "attendance", ["organization_id", "status", "attendance_date"], schema="attendance")
