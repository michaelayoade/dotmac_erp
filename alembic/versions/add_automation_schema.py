...
def upgrade() -> None:
    ...
    if not inspector.has_table("recurring_template", schema="automation"):
        op.create_table(
            ...
            sa.Column(
                "occurrences_count", sa.Integer, nullable=False, server_default="0"
            ),
            ...
            sa.Column(
                "days_before_due", sa.Integer, nullable=False, server_default="30"
            ),
            sa.Column(
        ...
        op.create_index(
            "idx_recurring_template_org",
            "recurring_template",
            ["organization_id"],
            schema="automation",
        )
        op.create_index(
            "idx_recurring_template_next_run",
            "recurring_template",
            ["next_run_date", "status"],
            schema="automation",
        )
        op.create_index(
            "idx_recurring_template_entity_type",
            "recurring_template",
            ["entity_type"],
            schema="automation",
        )
    ...
    if not inspector.has_table("recurring_log", schema="automation"):
        op.create_table(
            ...
            sa.ForeignKeyConstraint(
                ["template_id"],
                ["automation.recurring_template.template_id"],
                name="fk_recurring_log_template",
                ondelete="CASCADE",
            ),
            schema="automation",
        )
        op.create_index(
            "idx_recurring_log_template",
            "recurring_log",
            ["template_id"],
            schema="automation",
        )
        op.create_index(
            "idx_recurring_log_generated_at",
            "recurring_log",
            ["generated_at"],
            schema="automation",
        )
        op.create_index(
            "idx_recurring_log_status", "recurring_log", ["status"], schema="automation"
        )
    ...
    if not inspector.has_table("workflow_rule", schema="automation"):
        op.create_table(
            ...
            sa.Column(
                "stop_on_match", sa.Boolean, nullable=False, server_default="false"
            ),
            sa.Column(
                "execute_async", sa.Boolean, nullable=False, server_default="true"
            ),
            sa.Column(
            ...
            sa.UniqueConstraint(
                "organization_id", "rule_name", name="uq_workflow_rule_name"
            ),
            schema="automation",
        )
        op.create_index(
            "idx_workflow_rule_org",
            "workflow_rule",
            ["organization_id"],
            schema="automation",
        )
        op.create_index(
            "idx_workflow_rule_entity",
            "workflow_rule",
            ["entity_type"],
            schema="automation",
        )
        op.create_index(
            "idx_workflow_rule_trigger",
            "workflow_rule",
            ["trigger_event"],
            schema="automation",
        )
        op.create_index(
            "idx_workflow_rule_active",
            "workflow_rule",
            ["is_active"],
            schema="automation",
        )
    ...
    if not inspector.has_table("workflow_execution", schema="automation"):
        op.create_table(
            ...
            sa.ForeignKeyConstraint(
                ...
                name="fk_workflow_execution_rule",
                ondelete="CASCADE",
            ),
            schema="automation",
        )
        op.create_index(
            "idx_workflow_execution_rule",
            "workflow_execution",
            ["rule_id"],
            schema="automation",
        )
        op.create_index(
            "idx_workflow_execution_entity",
            "workflow_execution",
            ["entity_type", "entity_id"],
            schema="automation",
        )
        op.create_index(
            "idx_workflow_execution_triggered",
            "workflow_execution",
            ["triggered_at"],
            schema="automation",
        )
    ...
    if not inspector.has_table("custom_field_definition", schema="automation"):
        op.create_table(
            ...
            sa.Column(
                "show_in_list", sa.Boolean, nullable=False, server_default="false"
            ),
            sa.Column(
                "show_in_form", sa.Boolean, nullable=False, server_default="true"
            ),
            sa.Column(
                "show_in_detail", sa.Boolean, nullable=False, server_default="true"
            ),
            sa.Column(
                "show_in_print", sa.Boolean, nullable=False, server_default="false"
            ),
            sa.Column("is_active", sa.Boolean, nullable=False, server_default="true"),
            sa.Column("created_by", UUID(as_uuid=True), nullable=False),
            sa.Column(
            ...
            sa.UniqueConstraint(
                ...
                "entity_type",
                "field_code",
                name="uq_custom_field_code",
            ),
            schema="automation",
        )
        op.create_index(
            "idx_custom_field_org",
            "custom_field_definition",
            ["organization_id"],
            schema="automation",
        )
        op.create_index(
            "idx_custom_field_entity",
            "custom_field_definition",
            ["entity_type"],
            schema="automation",
        )
        op.create_index(
            "idx_custom_field_active",
            "custom_field_definition",
            ["is_active"],
            schema="automation",
        )
    ...
    if not inspector.has_table("document_template", schema="automation"):
        ...
        op.create_index(
            "idx_document_template_org",
            "document_template",
            ["organization_id"],
            schema="automation",
        )
        op.create_index(
            "idx_document_template_type",
            "document_template",
            ["template_type"],
            schema="automation",
        )
        op.create_index(
            "idx_document_template_default",
            "document_template",
            ["is_default"],
            schema="automation",
        )
