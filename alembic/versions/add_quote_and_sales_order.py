...
def upgrade() -> None:
    ...
    if not table_exists("quote", "ar"):
        op.create_table(
            ...
            sa.Column("customer_id", UUID(as_uuid=True), sa.ForeignKey("ar.customer.customer_id"), nullable=False),
            ...
            sa.Column("exchange_rate", sa.Numeric(19, 10), nullable=False, server_default="1"),
            sa.Column("payment_terms_id", UUID(as_uuid=True), sa.ForeignKey("ar.payment_terms.payment_terms_id"), nullable=True),
            ...
            sa.Column("converted_to_invoice_id", UUID(as_uuid=True), sa.ForeignKey("ar.invoice.invoice_id"), nullable=True),
            ...
            sa.UniqueConstraint("organization_id", "quote_number", name="uq_quote_number"),
            schema="ar",
        )
    if columns_exist("quote", "ar", ["organization_id", "status"]) and not index_exists(
        ...
        op.create_index("idx_quote_org_status", "quote", ["organization_id", "status"], schema="ar")
    ...
    if columns_exist("quote", "ar", ["organization_id", "quote_date"]) and not index_exists(
        "idx_quote_date",
        "quote",
        "ar",
        ["organization_id", "quote_date"],
    ):
        op.create_index("idx_quote_date", "quote", ["organization_id", "quote_date"], schema="ar")
    ...
    if not table_exists("quote_line", "ar"):
        op.create_table(
            ...
            sa.Column("quote_id", UUID(as_uuid=True), sa.ForeignKey("ar.quote.quote_id", ondelete="CASCADE"), nullable=False),
            ...
            sa.Column("discount_amount", sa.Numeric(19, 4), nullable=False, server_default="0"),
            sa.Column("tax_code_id", UUID(as_uuid=True), sa.ForeignKey("tax.tax_code.tax_code_id"), nullable=True),
            sa.Column("tax_amount", sa.Numeric(19, 4), nullable=False, server_default="0"),
            sa.Column("line_total", sa.Numeric(19, 4), nullable=False),
            sa.Column("revenue_account_id", UUID(as_uuid=True), sa.ForeignKey("gl.account.account_id"), nullable=True),
            sa.Column("project_id", UUID(as_uuid=True), sa.ForeignKey("core_org.project.project_id"), nullable=True),
            sa.Column("cost_center_id", UUID(as_uuid=True), sa.ForeignKey("core_org.cost_center.cost_center_id"), nullable=True),
            schema="ar",
        )
    ...
    if not table_exists("sales_order", "ar"):
        op.create_table(
            "sales_order",
            sa.Column("so_id", UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
            sa.Column("organization_id", UUID(as_uuid=True), nullable=False),
            sa.Column("so_number", sa.String(30), nullable=False),
            sa.Column("customer_po_number", sa.String(100), nullable=True),
            sa.Column("reference", sa.String(100), nullable=True),
            sa.Column("customer_id", UUID(as_uuid=True), sa.ForeignKey("ar.customer.customer_id"), nullable=False),
            sa.Column("quote_id", UUID(as_uuid=True), sa.ForeignKey("ar.quote.quote_id"), nullable=True),
            ...
            sa.Column("subtotal", sa.Numeric(19, 4), nullable=False, server_default="0"),
            sa.Column("discount_amount", sa.Numeric(19, 4), nullable=False, server_default="0"),
            sa.Column("shipping_amount", sa.Numeric(19, 4), nullable=False, server_default="0"),
            sa.Column("tax_amount", sa.Numeric(19, 4), nullable=False, server_default="0"),
            sa.Column("total_amount", sa.Numeric(19, 4), nullable=False, server_default="0"),
            sa.Column("invoiced_amount", sa.Numeric(19, 4), nullable=False, server_default="0"),
            sa.Column(
                "currency_code",
                sa.String(3),
                nullable=False,
            ...
            sa.Column("is_backorder", sa.Boolean, nullable=False, server_default="false"),
            sa.Column("allow_partial_shipment", sa.Boolean, nullable=False, server_default="true"),
            sa.Column("internal_notes", sa.Text, nullable=True),
            sa.Column("customer_notes", sa.Text, nullable=True),
            ...
            sa.Column("updated_by", UUID(as_uuid=True), nullable=True),
            sa.Column("updated_at", sa.DateTime(timezone=True), nullable=True),
            sa.UniqueConstraint("organization_id", "so_number", name="uq_sales_order_number"),
            schema="ar",
        )
    if columns_exist("sales_order", "ar", ["organization_id", "status"]) and not index_exists(
        "idx_so_org_status",
        "sales_order",
        "ar",
        ["organization_id", "status"],
    ):
        op.create_index("idx_so_org_status", "sales_order", ["organization_id", "status"], schema="ar")
    ...
    if columns_exist("sales_order", "ar", ["organization_id", "order_date"]) and not index_exists(
        "idx_so_date",
        "sales_order",
        "ar",
        ["organization_id", "order_date"],
    ):
        op.create_index("idx_so_date", "sales_order", ["organization_id", "order_date"], schema="ar")
    ...
    if columns_exist("quote", "ar", ["converted_to_so_id"]) and not fk_exists(
        ...
        op.create_foreign_key(
            "fk_quote_converted_to_so",
            "quote", "sales_order",
            ["converted_to_so_id"], ["so_id"],
            source_schema="ar", referent_schema="ar",
        )
    ...
    if not table_exists("sales_order_line", "ar"):
        op.create_table(
            "sales_order_line",
            sa.Column("line_id", UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
            sa.Column("so_id", UUID(as_uuid=True), sa.ForeignKey("ar.sales_order.so_id", ondelete="CASCADE"), nullable=False),
            sa.Column("line_number", sa.Integer, nullable=False),
            sa.Column("item_id", UUID(as_uuid=True), sa.ForeignKey("inv.item.item_id"), nullable=True),
            sa.Column("item_code", sa.String(50), nullable=True),
            sa.Column("description", sa.String(500), nullable=False),
            sa.Column("quantity_ordered", sa.Numeric(19, 4), nullable=False),
            sa.Column("quantity_shipped", sa.Numeric(19, 4), nullable=False, server_default="0"),
            sa.Column("quantity_invoiced", sa.Numeric(19, 4), nullable=False, server_default="0"),
            sa.Column("quantity_backordered", sa.Numeric(19, 4), nullable=False, server_default="0"),
            sa.Column("unit_of_measure", sa.String(20), nullable=True),
            sa.Column("unit_price", sa.Numeric(19, 4), nullable=False),
            sa.Column("discount_percent", sa.Numeric(5, 2), nullable=False, server_default="0"),
            sa.Column("discount_amount", sa.Numeric(19, 4), nullable=False, server_default="0"),
    ...
    if not table_exists("shipment", "ar"):
        op.create_table(
            "shipment",
            sa.Column("shipment_id", UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
            sa.Column("organization_id", UUID(as_uuid=True), nullable=False),
            sa.Column("shipment_number", sa.String(30), nullable=False),
            sa.Column("so_id", UUID(as_uuid=True), sa.ForeignKey("ar.sales_order.so_id"), nullable=False),
            ...
            sa.Column("created_by", UUID(as_uuid=True), nullable=False),
            sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
            sa.UniqueConstraint("organization_id", "shipment_number", name="uq_shipment_number"),
            schema="ar",
        )
    ...
    if columns_exist("shipment", "ar", ["organization_id", "shipment_date"]) and not index_exists(
        "idx_shipment_date",
        "shipment",
        "ar",
        ["organization_id", "shipment_date"],
    ):
        op.create_index("idx_shipment_date", "shipment", ["organization_id", "shipment_date"], schema="ar")

    # Create shipment_line table
    if not table_exists("shipment_line", "ar"):
        op.create_table(
            "shipment_line",
            sa.Column("shipment_line_id", UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
            sa.Column("shipment_id", UUID(as_uuid=True), sa.ForeignKey("ar.shipment.shipment_id", ondelete="CASCADE"), nullable=False),
            sa.Column("so_line_id", UUID(as_uuid=True), sa.ForeignKey("ar.sales_order_line.line_id"), nullable=False),
    ...
    if columns_exist("shipment_line", "ar", ["shipment_id"]) and not index_exists(
        ...
        op.create_index("idx_shipment_line_shipment", "shipment_line", ["shipment_id"], schema="ar")
    if columns_exist("shipment_line", "ar", ["so_line_id"]) and not index_exists(
        ...
        op.create_index("idx_shipment_line_so_line", "shipment_line", ["so_line_id"], schema="ar")
...
def downgrade() -> None:
    ...
    op.drop_constraint("fk_quote_converted_to_so", "quote", schema="ar", type_="foreignkey")
