...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('content_type', sa.String(100), nullable=False, default='application/pdf', comment='MIME type'),
        sa.Column('file_size_bytes', sa.BigInteger(), nullable=False, default=0),
        sa.Column('content_hash', sa.String(64), nullable=True, comment='SHA256 hash for integrity verification'),
        sa.Column('effective_date', sa.Date(), nullable=False, comment='Date this version becomes effective'),
        sa.Column('expiry_date', sa.Date(), nullable=True, comment='Optional expiry date'),
        sa.Column('requires_acknowledgment', sa.Boolean(), nullable=False, default=True, comment='Whether employees must acknowledge this document'),
        ...
        sa.ForeignKeyConstraint(['previous_version_id'], ['hr.hr_document.document_id'], ),
        sa.PrimaryKeyConstraint('document_id'),
        sa.UniqueConstraint('organization_id', 'document_code', 'version', name='uq_hr_document_code_version'),
        schema='hr'
    )
    op.create_index('idx_hr_document_org', 'hr_document', ['organization_id'], schema='hr')
    op.create_index('idx_hr_document_category', 'hr_document', ['category'], schema='hr')
    ...
    op.create_table(
        ...
        sa.Column('acknowledgment_id', postgresql.UUID(as_uuid=True), server_default=sa.text('gen_random_uuid()'), nullable=False),
        sa.Column('document_id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('employee_id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('acknowledged_at', sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.Column('ip_address', sa.String(45), nullable=True, comment='IP address at time of acknowledgment'),
        sa.Column('user_agent', sa.String(500), nullable=True, comment='Browser user agent for audit'),
        sa.Column('signature_data', sa.Text(), nullable=True, comment='Base64 encoded signature image if captured'),
        sa.Column('confirmation_text', sa.Text(), nullable=True, comment='Text the employee confirmed'),
        sa.ForeignKeyConstraint(['document_id'], ['hr.hr_document.document_id'], ),
        sa.ForeignKeyConstraint(['employee_id'], ['hr.employee.employee_id'], ),
        sa.PrimaryKeyConstraint('acknowledgment_id'),
        sa.UniqueConstraint('document_id', 'employee_id', name='uq_hr_doc_ack_document_employee'),
        schema='hr'
    )
    op.create_index('idx_hr_doc_ack_document', 'hr_document_acknowledgment', ['document_id'], schema='hr')
    op.create_index('idx_hr_doc_ack_employee', 'hr_document_acknowledgment', ['employee_id'], schema='hr')


def downgrade() -> None:
    op.drop_index('idx_hr_doc_ack_employee', table_name='hr_document_acknowledgment', schema='hr')
    op.drop_index('idx_hr_doc_ack_document', table_name='hr_document_acknowledgment', schema='hr')
